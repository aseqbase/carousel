...
class Carousel extends Collection
{
	...
	/**
	 * To show everythings as a slideshow
	 * @param null|array $items The items to show in the carousel, if not set, the Items property will be used
	 * @example
	 * $Carousel = new MiMFa\Module\Carousel([
	 *     ["Title"=>"First Slide", "Description"=>"This is the first slide", "Image"=>"path/to/image1.jpg"],
	 *     ["Title"=>"Second Slide", "Description"=>"This is the second slide", "Image"=>"path/to/image2.jpg"],
	 * ]);
	 */
	...
	public function GetInner()
	{
		return join(PHP_EOL, iterator_to_array((function () {
			...
			foreach (Convert::ToItems($this->Items) as $item) {
				...
				if (is_array($item) && get($item, "Image"))
					$inners[] = Struct::Media(
						Struct::Division(
					...
					$inners[] = Struct::Division($item, ["data-bs-target" => $target, "data-bs-slide-to" => $i, "class" => "carousel-item" . ($active ? " active" : "")]);
